...
class ParserModel(nn.Module):
    ...
    def __init__(self, embeddings, n_features=36,
        ...
        input_feature_size = self.n_features * self.embed_size
        
        self.embed_to_hidden_weight = nn.Parameter(nn.init.xavier_uniform_(torch.Tensor(input_feature_size, self.hidden_size)))
        self.embed_to_hidden_bias = nn.Parameter(nn.init.uniform_(torch.Tensor(self.hidden_size)))
        
        self.dropout = nn.Dropout(dropout_prob)

        self.hidden_to_logits_weight = nn.Parameter(nn.init.xavier_uniform_(torch.Tensor(self.hidden_size, n_classes)))
        self.hidden_to_logits_bias = nn.Parameter(nn.init.uniform_(torch.Tensor(n_classes)))
    ...
    def embedding_lookup(self, w):
        ...
        idxs = w.unsqueeze(2).repeat(1,1,self.embed_size) # idxs.shape = (batch_size, n_features, embed_size)
        
        x = self.embeddings.unsqueeze(0).repeat(batch_size, 1, 1).gather(1, idxs).view(batch_size, -1)
