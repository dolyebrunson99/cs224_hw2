...
class PartialParse(object):
    ...
    def parse_step(self, transition):
        ...
        match transition:
            case "S": # Shift
                # removes the first in the buffer and pushes it on to stack
                first = self.buffer[0]
                self.buffer = self.buffer[1:]
                self.stack.append(first)
                
            case "LA": # Left Arc
                # adding (second_word, first_word) dependencies
                # removes second_word from stack
                first_word, second_word = self.stack[-1], self.stack[-2]
                self.dependencies.append((first_word, second_word))
                del self.stack[-2]

            case "RA": # Right Arc
                # adding (first_word, second_word) dependencies
                # removes first_word from stack
                first_word, second_word = self.stack[-1], self.stack[-2]
                self.dependencies.append((second_word, first_word))
                del self.stack[-1]

            case _:
                raise "unrecognized transition"
...
def minibatch_parse(sentences, model, batch_size):
    ...
    while unfinished_parses:
        ...
        next_transitions = model.predict(first_batch)
        for partial_parse, next_transition in zip(first_batch, next_transitions):
            partial_parse.parse_step(next_transition)
